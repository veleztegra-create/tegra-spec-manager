...
const SpecsManager = (function() {
    // ========== CONFIGURACIÓN ==========
    const CONFIG = {
        STORAGE_PREFIX: 'tegra_spec_',  // ESTANDARIZADO: mismo prefijo que StorageManager
        AUTO_SAVE_INTERVAL: 120000, // 2 minutos
        MAX_SPECS: 100,
        BACKUP_ENABLED: true,
        SPEC_PREFIX: 'spec_' // Para identificar specs dentro del prefijo principal
    };
    ...
    function generateSpecId(style) {
        ...
        try {
            ...
            return `${CONFIG.SPEC_PREFIX}${stylePart}_${timestamp}_${random}`;
        } catch (error) {
            console.error('❌ Error en generateSpecId:', error);
            return `${CONFIG.SPEC_PREFIX}backup_${Date.now()}`;
        }
    }
    
    function getFullStorageKey(specId) {
        return `${CONFIG.STORAGE_PREFIX}${specId}`;
    }
    ...
    function getAllSpecKeys() {
        console.log('🔍 Obteniendo todas las keys de specs...');
        try {
            const allKeys = Object.keys(localStorage);
            const specKeys = allKeys.filter(key => 
                key.startsWith(CONFIG.STORAGE_PREFIX + CONFIG.SPEC_PREFIX)
            );
            
            console.log(`✅ Encontradas ${specKeys.length} specs en localStorage`);
            return specKeys;
        } catch (error) {
            console.error('❌ Error en getAllSpecKeys:', error);
            return [];
        }
    }
    ...
    function saveCurrentSpec() {
        ...
        try {
            ...
            const specId = generateSpecId(style);
            const storageKey = getFullStorageKey(specId);
    ...
    function loadSavedSpecsList() {
        ...
        try {
            ...
            console.log(`✅ Lista de specs cargada: ${specKeys.length} specs`);
    ...
    function autoSave() {
        ...
        try {
            ...
            if (style || customer) {
                const data = collectData();
                if (validateSpecData(data)) {
                    const specId = `autosave_${Date.now()}`;
                    const backupKey = getFullStorageKey(specId);
    ...
    function migrateOldSpecs() {
        console.log('🔄 Verificando migración de specs antiguas...');
        
        const oldSpecs = Object.keys(localStorage).filter(k => 
            k.startsWith('spec_') && !k.startsWith('tegra_spec_')
        );
        
        if (oldSpecs.length > 0) {
            console.log(`🔧 Migrando ${oldSpecs.length} specs antiguas...`);
            
            oldSpecs.forEach(oldKey => {
                try {
                    const data = localStorage.getItem(oldKey);
                    const newKey = 'tegra_' + oldKey;
                    localStorage.setItem(newKey, data);
                    console.log(`✅ Migrado: ${oldKey} → ${newKey}`);
                    
                    // Opcional: eliminar el antiguo
            ...
            showStatus(`✅ ${oldSpecs.length} specs antiguas migradas`, 'success');
        }
    }
    ...
    function init() {
        ...
        // Migrar specs antiguas
        setTimeout(migrateOldSpecs, 2000);
