...
(function () {
  ...
  function buildPlacementHtml(placement, index, total, data) {
    ...
    const colorAdditives = uniqueDesignColors
      .map((c) => String(c.additives || '').trim())
      .filter(Boolean);
    const uniqueAdditives = Array.from(new Set(colorAdditives));
    const additivesSummary = uniqueAdditives.length
      ? `<div class="colors-additives-note"><strong>ADITIVOS:</strong> ${esc(uniqueAdditives.join(' · '))}</div>`
      : '<div class="colors-additives-note"><strong>ADITIVOS:</strong> Según preset/reglas del sistema.</div>';
    ...
    return `
      <section class="placement-section">
        ...
        <div class="colors-section"><h3 class="sub-title">Colores y Tintas</h3><div class="colors-grid">${colors}</div>${additivesSummary}</div>
    ...
    const pagesHtml = pages.map((page, pageIndex) => {
      const labelsHtml = page.map((formula) => {
        ...
        const additivesHtml = additives
          .map((item) => `<span class="lab-mini-pill">ADITIVOS: ${esc(item.name || item.label)} · ${esc(item.display)}</span>`)
          .join('');
        ...
        return `
          <article class="lab-label-card">
            ...
            <div class="lab-mini-pills">${additivesHtml || '<span class="lab-mini-pill">ADITIVOS: Sin aditivos</span>'}</div>
          </article>
        `;
      }).join('');
